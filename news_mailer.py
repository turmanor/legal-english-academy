...
def build_pdf_placeholder(path: str, now_bj: datetime) -> None:
    ...
    with open(path, "wb") as file:
        file.write(content.encode("latin-1", "ignore"))
...
def send_mail() -> None:
    ...
    try:
        ...
        raise RuntimeError(f"DNS lookup failed for SMTP host '{smtp_host}': {error}") from error
    except smtplib.SMTPAuthenticationError as error:
        raise RuntimeError("SMTP authentication failed. Check SMTP_USER/SMTP_PASS authorization settings.") from error
    finally:
